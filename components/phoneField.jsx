...
export default function PhoneField({ className, onChange, value, ...props }) {
  const id = "data"
  ...
  // Only render the phone input component on the client
  if (!isClient) {
    return (
...
const PhoneInput = ({ className, ...props }) => {
  return (
    <Input
      ...
      className={cn("-ms-px rounded-s-none shadow-none focus-visible:z-10 border border-muted", className)}
      {...props} />
  );
}
...
const FlagComponent = ({ country, countryName }) => {
  ...
  return (
    <span className="w-5 overflow-hidden rounded-sm text-primary">
      {Flag ? (
